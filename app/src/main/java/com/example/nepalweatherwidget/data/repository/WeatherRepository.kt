...
class WeatherRepository @Inject constructor(
    ...
    override suspend fun getWeatherData(location: String): Result<DomainWeatherData> = runCatching {
        Log.d(TAG, "Fetching weather data for location: $location")
        val (latitude, longitude) = getCoordinatesForLocation(location)
        getWeatherData(latitude, longitude).getOrThrow()
    }

    override suspend fun getWeatherData(latitude: Double, longitude: Double): Result<DomainWeatherData> = runCatching {
        ...
        val domainWeather = weather.toDomainModel()
        cachedWeather = domainWeather
        domainWeather
    }

    override suspend fun getCachedWeatherData(): Result<DomainWeatherData> = runCatching {
        Log.d(TAG, "Getting cached weather data")
        cachedWeather ?: throw Exception("No cached weather data available")
    }

    override suspend fun getAirQuality(): Result<AirQuality> = runCatching {
        Log.d(TAG, "Fetching air quality data")
        val (latitude, longitude) = getCoordinatesForLocation("Kathmandu")
        getAirQuality(latitude, longitude).getOrThrow()
    }

    override suspend fun getAirQuality(latitude: Double, longitude: Double): Result<AirQuality> = runCatching {
        ...
        val domainAirQuality = airQuality.toDomainModel()
        cachedAirQuality = domainAirQuality
        domainAirQuality
    }

    override suspend fun getWeatherAndAirQuality(location: String): Result<Pair<DomainWeatherData, AirQuality>> = runCatching {
        Log.d(TAG, "Fetching weather and air quality data for location: $location")
        val (latitude, longitude) = getCoordinatesForLocation(location)
        getWeatherAndAirQuality(latitude, longitude).getOrThrow()
    }

    override suspend fun getWeatherAndAirQuality(latitude: Double, longitude: Double): Result<Pair<DomainWeatherData, AirQuality>> = runCatching {
